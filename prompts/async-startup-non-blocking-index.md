...
5. **Add a helper `_not_ready_response`** and gate the two blocking handlers on it. Insert the helper above `search`:
   ...
   In `async def search(request: Request) -> JSONResponse`, insert at the top of the `try` block (before the `q` parameter check — we want a 503 even if the client also forgot `q`; it's a clearer signal during cold start):
   ...
   Same insertion at the top of the `try` block of `async def duplicates(request: Request) -> JSONResponse`. Do NOT remove the existing `run_in_threadpool(...)` wrappers or the rest of the handler bodies.
...
9. **Add three tests** in `tests/test_http_server.py`. Each new test MUST reset module state in a `finally` so it does not leak into other tests.
   ...
   **Important about existing tests:** the existing `test_search_with_query`, `test_duplicates_with_file`, `test_search_runs_in_threadpool`, `test_duplicates_runs_in_threadpool`, `test_duplicates_indexer_returns_error_dict_returns_400`, and `test_reindex_post` all set `mock_get.return_value = mock_indexer` and then hit endpoints expecting 200. Those tests will now fail because the handlers check `_indexer_ready` before calling `get_indexer()`. You MUST update each of these tests to ALSO set:

   ```python
   import semantic_search.http_server as http_server
   ready_event = asyncio.Event()
   ready_event.set()
   # save/restore as in the health tests
   http_server._indexer_ready = ready_event
   http_server._indexer = mock_indexer
   ```

   around the `TestClient` block, with a `try/finally` to restore the originals. Apply this to: `test_search_with_query`, `test_search_runs_in_threadpool`, `test_duplicates_with_file`, `test_duplicates_runs_in_threadpool`, `test_duplicates_indexer_returns_error_dict_returns_400`, `test_reindex_post`, and `test_health_returns_ok` (the existing one).
...
11. **Update `CHANGELOG.md`** under `## Unreleased` (create the section if missing, above `## v0.7.1`):
