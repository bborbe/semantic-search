...
If you ran one instance over one path set, the migration is three edits — a "scope" is just a name for the path list you already had:

1. Write a scope map naming those paths:

   ```yaml
   scopes:
     default:
       - /the/path/you/had/in/CONTENT_PATH
